...
def tracker(tracker_id, uid):
    if request.method=="GET":
        ...
        if tracker.t_type == "Mood":
            ...
            return render_template("misc_mood.html", tracker=tracker, trackers=trackers, mlist=mlist, uid=uid, uname=uname, imgloc=imgloc)
        else:
            return render_template("Mood.html", tracker=tracker, trackers=trackers, linkers=linkers, uid=uid, uname=uname, imgloc=imgloc)
